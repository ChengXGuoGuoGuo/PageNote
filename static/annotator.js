(()=>{
  ...
  const state={notes:[],project:null,author:'',composing:false,editing:null};
  ...
  shadow.innerHTML=`
    ...
    <div class="layer">
      <div class="outline"></div><div class="hint" role="status">点击页面元素添加批注 · Esc 退出</div><div class="markers"></div><div class="editor-slot"></div>
  ...
  const setCompose=active=>{state.composing=active;els.fab.classList.toggle('active',active);els.hint.classList.toggle('show',active);els.outline.classList.remove('show');document.documentElement.style.cursor=active?'crosshair':'';post({type:'mode',active})};
  ...
  const showEditor=(target,x,y,note=null)=>{
  ...
  const onMove=event=>{if(!state.composing||isUi(event.target))return;const rect=event.target.getBoundingClientRect();Object.assign(els.outline.style,{left:`${rect.left}px`,top:`${rect.top}px`,width:`${rect.width}px`,height:`${rect.height}px`});els.outline.classList.add('show')};
  const onClick=event=>{if(!state.composing||isUi(event.target))return;event.preventDefault();event.stopPropagation();setCompose(false);showEditor(event.target,event.clientX,event.clientY)};
  ...
  document.addEventListener('pointermove',onMove,true);document.addEventListener('click',onClick,true);document.addEventListener('keydown',event=>{if(event.key==='Escape'){setCompose(false);els.slot.innerHTML=''}});addEventListener('scroll',renderMarkers,true);addEventListener('resize',renderMarkers);
  ...
  window.PageNote={toggleCompose:()=>{closePanel();setCompose(!state.composing)},open:openPanel,destroy:()=>{observer.disconnect();document.removeEventListener('pointermove',onMove,true);document.removeEventListener('click',onClick,true);host.remove()},getNotes:()=>structuredClone(state.notes)};
