(()=>{
  ...
  const cancelInteraction=()=>{setCompose(false);els.slot.innerHTML='';state.editing=null;state.selection=null;state.drag=null;els.outline.classList.remove('show')};
  ...
  document.addEventListener('pointerdown',onPointerDown,true);document.addEventListener('pointermove',onMove,true);document.addEventListener('pointerup',onPointerUp,true);document.addEventListener('pointercancel',onPointerUp,true);document.addEventListener('click',onClick,true);document.addEventListener('keydown',event=>{if(event.key==='Escape')cancelInteraction()});addEventListener('scroll',renderPositions,true);addEventListener('resize',renderPositions);
  ...
  addEventListener('message',event=>{if(event.source!==parent||event.data?.source!=='pagenote-host')return;const message=event.data;if(message.type==='sync'){state.notes=Array.isArray(message.notes)?message.notes:[];state.project=message.project||null;state.author=message.author||state.author;els.title.textContent=state.project?.title||'页面批注';render()}if(message.type==='author')state.author=message.author||'';if(message.type==='command'&&message.command==='toggleCompose'){state.author=message.author||state.author;closePanel();setCompose(!state.composing)}if(message.type==='command'&&message.command==='cancelCompose')cancelInteraction()});
